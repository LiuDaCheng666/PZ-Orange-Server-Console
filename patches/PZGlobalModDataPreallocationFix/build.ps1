...
$output = Join-Path $build 'PZGlobalModDataPreallocationFix-agent.jar'
...
& (Join-Path $jdk 'javac.exe') -encoding UTF-8 -cp "$asmJar;$gameJar" -d $mainClasses `
    (Join-Path $root 'src\main\java\cn\zombiecommunity\pzglobalmoddata\PZGlobalModDataPreallocationAgent.java') `
    (Join-Path $root 'src\main\java\cn\zombiecommunity\pzglobalmoddata\GlobalModDataPreallocationRuntime.java')
...
& (Join-Path $jdk 'javac.exe') -encoding UTF-8 -cp "$mainClasses;$asmJar;$gameJar" -d $testClasses `
    (Join-Path $root 'src\test\java\cn\zombiecommunity\pzglobalmoddata\CapacityCalculationTest.java') `
    (Join-Path $root 'src\test\java\cn\zombiecommunity\pzglobalmoddata\TransformSmokeTest.java') `
    (Join-Path $root 'src\test\java\cn\zombiecommunity\pzglobalmoddata\GameClassLoadSmokeTest.java')
if ($LASTEXITCODE -ne 0) { throw 'test javac failed' }
& (Join-Path $jdk 'java.exe') -cp "$testClasses;$mainClasses;$asmJar;$gameJar" `
    cn.zombiecommunity.pzglobalmoddata.CapacityCalculationTest
if ($LASTEXITCODE -ne 0) { throw 'capacity calculation test failed' }
& (Join-Path $jdk 'java.exe') -cp "$testClasses;$mainClasses;$asmJar;$gameJar" `
    cn.zombiecommunity.pzglobalmoddata.TransformSmokeTest $gameJar
...
& (Join-Path $jdk 'java.exe') -Xverify:all `
    "-javaagent:$output=enabled=true,headroomBytes=4194304,maxPreallocateBytes=268435456" `
    -cp "$testClasses;$gameJar;$output" cn.zombiecommunity.pzglobalmoddata.GameClassLoadSmokeTest
