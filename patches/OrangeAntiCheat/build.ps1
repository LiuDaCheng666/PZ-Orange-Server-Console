...
$OutputJar = Join-Path $BuildDir 'OrangeAntiCheat-agent.jar'

foreach ($required in @($Javac, $Java, $Jar, $GameJar, $AsmJar)) {
...
$Sources = Get-ChildItem -LiteralPath (Join-Path $ProjectRoot 'src') -Filter '*.java' -File |
    Select-Object -ExpandProperty FullName
& $Javac --release 25 -encoding UTF-8 -cp "$GameJar;$AsmJar" -d $ClassesDir $Sources
if ($LASTEXITCODE -ne 0) { throw "javac failed with exit code $LASTEXITCODE" }

$TestSources = Get-ChildItem -LiteralPath (Join-Path $ProjectRoot 'test') -Filter '*.java' -Recurse |
    Select-Object -ExpandProperty FullName
