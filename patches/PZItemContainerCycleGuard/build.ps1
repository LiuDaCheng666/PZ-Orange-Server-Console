...
$OutputJar = Join-Path $BuildDir 'PZItemContainerCycleGuard-agent.jar'
...
& $Java -Xverify:all -cp "$GameJar;$AsmJar;$ClassesDir;$TestClassesDir" `
    cn.zombiecommunity.pzcontainerguard.TransformSmokeTest $GameJar
...
& $Java -Xverify:all "-javaagent:$OutputJar" -cp "$GameJar;$OutputJar;$TestClassesDir" `
    cn.zombiecommunity.pzcontainerguard.CycleIntegrationTest
if ($LASTEXITCODE -ne 0) { throw "cycle integration test failed with exit code $LASTEXITCODE" }
