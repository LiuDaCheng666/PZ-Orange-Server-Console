...
$OutputJar = Join-Path $BuildDir 'PZSelectiveWorldResetGuard-agent.jar'

foreach ($path in @($Javac, $Java, $Jar, $GameJar, $AsmJar)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing build dependency: $path" }
}
...
$TestSources = Get-ChildItem -LiteralPath (Join-Path $ProjectRoot 'src\test\java') -Filter '*.java' -Recurse |
    Select-Object -ExpandProperty FullName
& $Javac --release 25 -encoding UTF-8 -cp "$GameJar;$AsmJar;$ClassesDir" -d $TestClassesDir $TestSources
if ($LASTEXITCODE -ne 0) { throw "test javac failed with exit code $LASTEXITCODE" }

& $Java -Xverify:all -cp "$GameJar;$AsmJar;$ClassesDir;$TestClassesDir" `
    cn.zombiecommunity.pzresetguard.TransformSmokeTest $GameJar
...
& $Java -Xverify:all -cp "$GameJar;$ClassesDir;$TestClassesDir" `
    cn.zombiecommunity.pzresetguard.ManifestFormatTest
if ($LASTEXITCODE -ne 0) { throw "manifest test failed with exit code $LASTEXITCODE" }
...
& $Java -Xverify:all "-javaagent:$OutputJar" `
    -cp "$GameJar;$OutputJar;$TestClassesDir" `
    cn.zombiecommunity.pzresetguard.GameClassLoadSmokeTest
