...
- 管理员物品模板库可保存带附件与属性的物品快照并向不同服务器发放；选择性地图刷新可自动保护安全屋、人物和畜牧区，执行前创建并逐文件校验完整存档 ZIP，并支持停服一键完整回滚。
...
仓库同时公开当前三个服务器实际常驻的 8 个 Java Agent，包括反作弊、
ObjectModData 同步防护、玻璃附件死循环、容器循环、重复实体注册、尸体容器 ID、
联机长读条与动态贴图映射修复。每个目录都含源码、测试、构建脚本和当前部署 JAR。
