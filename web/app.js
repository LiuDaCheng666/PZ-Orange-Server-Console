...
let vaultSnapshot=null,vaultBusy=false,vaultSelectedId='',vaultTargetPlayers=[],vaultReceiptTimer=null;
...
let communitySnapshot=null,communityBusy=false,communityLedgerPage=1,communityLedgerPageCount=1,communityPollTimer=null,communityReceiptTimer=null,communityPendingRequestId='',communityHydratedRevision='';
...
function renderVaultPlayerOptions(){
  const select=document.querySelector('#vaultTargetPlayer');if(!select)return;
  select.innerHTML='<option value="">请选择已登记 SteamID 的玩家</option>'+vaultTargetPlayers.map((player,index)=>`<option value="${index}">${escapeHtml(player.username)} · ${escapeHtml(player.steamId)}${player.online?' · 在线':' · 历史'}</option>`).join('');
  select.disabled=!vaultTargetPlayers.length;
  document.querySelector('#vaultPlayerHint').textContent=vaultTargetPlayers.length?`已读取 ${vaultTargetPlayers.length} 条用户名与 SteamID 配对记录。`:'该服务器没有可用于安全发放的玩家身份记录。';
}
async function loadVaultPlayers(serverId){
  ...
  document.querySelector('#vaultPlayerHint').textContent='正在核验用户名与 SteamID 配对记录...';vaultTargetPlayers=[];
  try{const data=await api(`/api/players?serverId=${encodeURIComponent(serverId)}`);const current=document.querySelector('#vaultDestination');if(!current||current.value!==serverId)return;vaultTargetPlayers=(data.players||[]).filter(player=>/^7656119\d{10}$/.test(String(player.steamId||''))).sort((a,b)=>Number(b.online)-Number(a.online)||a.username.localeCompare(b.username,'zh-CN'));renderVaultPlayerOptions()}catch(error){if(select){select.innerHTML='<option value="">玩家目录读取失败</option>'}document.querySelector('#vaultPlayerHint').textContent=error.message}
}
...
async function submitVaultGrant(event){
  event.preventDefault();const template=(vaultSnapshot?.templates||[]).find(item=>item.templateId===vaultSelectedId),form=event.currentTarget,playerIndex=form.elements.player.value,player=playerIndex===''?null:vaultTargetPlayers[Number(playerIndex)],serverId=form.elements.serverId.value,count=Number(form.elements.count.value);
  if(!template||!player){toast('请选择保险库模板和目标玩家。',true);return}const profile=vaultSnapshot.profiles.find(item=>item.id===serverId);
  if(!confirm(`确认向 ${profile?.name||serverId} 的 ${player.username}（${player.steamId}）复制发放 ${count} 份“${vaultTemplateName(template)}”？\n\n原物品和容器内容不会被移除；目标服缺少任一物品或附件 Mod 时会整份拒绝发放。`))return;
  const button=form.querySelector('button[type="submit"]');button.disabled=true;
  try{const data=await api('/api/admin-item-vault/grant',{method:'POST',timeoutMs:30000,body:JSON.stringify({templateId:template.templateId,serverId,targetUsername:player.username,targetSteamId:player.steamId,count,confirm:'GRANT_ADMIN_VAULT_ITEM'})});vaultSnapshot.grants=[data.grant,...(vaultSnapshot.grants||[])];renderVaultHistory();scheduleVaultReceiptPoll();toast(data.message)}catch(error){toast(error.message,true)}finally{button.disabled=false}
}
...
function scheduleVaultReceiptPoll(){
  clearTimeout(vaultReceiptTimer);const pending=(vaultSnapshot?.grants||[]).filter(grant=>['queued','queued_offline'].includes(grant.status)).slice(0,10);if(!pending.length||activeView!=='vault')return;
  vaultReceiptTimer=setTimeout(async()=>{try{const results=await Promise.all(pending.map(grant=>api(`/api/admin-item-vault/receipt?id=${encodeURIComponent(grant.requestId)}`).catch(()=>null)));results.filter(Boolean).forEach(data=>{const index=vaultSnapshot.grants.findIndex(grant=>grant.requestId===data.grant.requestId);if(index>=0)vaultSnapshot.grants[index]=data.grant});renderVaultHistory()}finally{scheduleVaultReceiptPoll()}},2500);
}
...
const communityOperationLabels={
  ...
  admin_update_crisis:'修改危机参数',admin_start_bridge_battle:'启动大桥保卫战',
};
...
function renderCommunityBridge(){
  ...
  document.querySelectorAll('#view-community .community-admin-form input,#view-community .community-admin-form select,#view-community .community-admin-form button').forEach(control=>control.disabled=!writable);
}
function renderCommunityGovernance(){
  ...
  document.querySelector('#communityGovernmentForm').textContent=communityGovernmentLabels[law.governmentForm]||law.governmentForm||'--';
  ...
  const electionLabel={idle:'未进行',nomination:'提名阶段',voting:'投票阶段',tie:'平票加赛',completed:'已结算'}[election.phase]||election.phase||'未进行';
  document.querySelector('#communityLawSummary').innerHTML=`<div class="community-law-grid"><div><span>政体</span><strong>${escapeHtml(communityGovernmentLabels[law.governmentForm]||law.governmentForm||'--')}</strong></div><div><span>投票期</span><strong>${Number(law.votingDays||0)||'--'} 游戏日</strong></div><div><span>官员任期</span><strong>${Number(law.termDays||0)||'--'} 游戏日</strong></div><div><span>投票费用</span><strong>${Number(law.voteFee||0).toLocaleString('zh-CN')} 橙币</strong></div><div><span>当前选举</span><strong>${escapeHtml(electionLabel)}</strong></div><div><span>法律版本</span><strong>${Number(law.version||governance.revision||0)}</strong></div></div>`;
}
...
function renderCommunityCommands(){
  ...
  list.innerHTML=rows.length?rows.map(row=>{const status=String(row.status||'waiting'),success=communitySuccessStatuses.has(status),failed=['failed','error','rejected','expired'].includes(status);return `<article class="community-command-row" data-state="${escapeHtml(status)}"><span><b class="community-command-state ${success?'success':failed?'failed':'waiting'}">${escapeHtml(communityStatusLabels[status]||status)}</b><strong>${escapeHtml(communityOperationLabels[row.operation]||row.operation||'未知操作')}</strong><small>${formatCommunityTime(row.createdMs)} · ${escapeHtml(row.requestedBy||'未知管理员')}</small></span><code>${escapeHtml(String(row.requestId||'').slice(-16)||'--')}</code><p>${escapeHtml(row.detail||row.code||row.reason||'等待 Mod 返回执行结果。')}</p></article>`}).join(''):'<p class="empty-state compact">当前没有社区命令。</p>';
}
function renderCommunity(){
  if(!communitySnapshot)return;renderCommunityBridge();renderCommunityGovernance();renderCommunityTreasury();renderCommunityCrisis();hydrateCommunityForms();renderCommunityCommands();lucide.createIcons();
}
...
function showView(name){
  ...
  document.querySelector('#mobileCurrentTitle').textContent=titles[name];
  setMobileMenu(false);
...
function getAntiCheatExplanation(event={}){
  ...
  else if(event.type==='observed-health-sync')explanation=event.reason==='target_not_owned_by_connection'?'观察到连接提交了不属于自身玩家的健康同步。Agent 只写日志，没有拦截或修改数据，需人工复核。':event.reason==='non_finite_health'?'观察到客户端健康同步包含 NaN 或 Infinity。Agent 只写日志，没有拦截或修改数据，需结合玩家状态复核。':'观察到客户端上报的身体部位生命出现较大增加。自然恢复、治疗或 Mod 行为也可能触发；Agent 没有拦截，不能凭此单独定性。';
...
function renderAntiCheatSignalGuide(){
  ...
  const items=[
    ...
    {icon:'heart-pulse',tone:'warning',label:'健康同步异常（仅记录）',source:'OrangeAntiCheat · Health Audit',value:Number(summary.observedHealthSyncs||0),description:'记录较大增血、非有限值或目标异常；没有拦截，正常恢复和治疗也可能触发，必须人工复核。'},
...
const lifecycleStageLabels={queued:'等待后台执行',locking:'取得操作锁',"waiting-lifecycle-lock":'等待其他服务器维护',notifying:'发送维护通知',countdown:'等待玩家准备',saving:'保存世界',quitting:'正常退出',"waiting-stop":'等待 Java 结束',stabilizing:'释放内存与端口',updating:'SteamCMD 更新中',"verifying-update":'校验更新结果',starting:'启动新实例',"waiting-running":'等待新 Java 运行',completed:'操作完成',failed:'操作失败'};
const lifecycleStageMessages={queued:'操作已提交，等待后台执行器启动。',locking:'正在取得服务器生命周期操作锁。',"waiting-lifecycle-lock":'另一台服务器正在维护，本服务器将在前一项完成后自动继续。',notifying:'正在发送原生全服广播和右下角弹窗。',countdown:'维护通知已发送，正在等待玩家准备。',saving:'正在保存世界，并等待受控通道确认。',quitting:'保存已确认，正在请求服务器正常退出。',"waiting-stop":'退出命令已送达，正在等待旧 Java 进程完全结束，最长等待 15 分钟。',stabilizing:'旧 Java 已结束，正在等待大内存、端口和 Steam 网络资源完全释放。',updating:'正在通过 SteamCMD 更新 AppID 380870 的 public 分支。',"verifying-update":'SteamCMD 已结束，正在校验 BuildID 和成功标记。',starting:'资源释放缓冲已结束，正在启动新实例。',"waiting-running":'启动脚本已执行，正在等待新 Java 进入运行状态。',completed:'操作已完成。',failed:'操作失败，请查看下方原因。'};
...
function renderMapReset(data,hydrateConfig=false){
  ...
  else if(status.state==='completed'){badge.textContent=status.mode==='apply'?'刷新完成':status.mode==='rollback'?'回滚完成':'审计完成';badge.className='badge running';alert.dataset.state='neutral';alertTitle.textContent=status.mode==='apply'?'完整备份已校验，区块已隔离':status.mode==='rollback'?'完整存档已恢复':'只读审计已完成';alertText.textContent=status.mode==='apply'?'服务器保持停止状态；确认结果后再手动启动。':status.mode==='rollback'?'回滚前的当前世界已另存，服务器仍保持停止。':'审计没有修改存档。当前配置未变化时可在停服后正式执行。'}
...
function renderChunkRecovery(data,hydrate=false){
  ...
  const oldTransaction=chunkTransaction.value;const transactions=(data.transactions||[]).filter(item=>item.state==='completed');chunkTransaction.innerHTML=transactions.length?transactions.map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(formatDate(item.createdAt))} · ${escapeHtml((item.chunks||[]).join('; '))}</option>`).join(''):'<option value="">没有可回滚事务</option>';if(oldTransaction&&transactions.some(item=>item.id===oldTransaction))chunkTransaction.value=oldTransaction;
  ...
  else if(status.state==='completed'){badge.textContent=status.mode==='restore'?'恢复完成':'回滚完成';badge.className='badge running';alert.dataset.state='neutral';title.textContent=status.mode==='restore'?'定点区块已恢复':'恢复事务已撤销';text.textContent='服务器仍保持停止，请启动后验证玩家建筑、容器和物品。'}
...
function updateChunkRecoveryButtons(){const data=chunkRecoverySnapshot,busy=['running','finalizing'].includes(data?.status?.state);document.querySelector('#runChunkAudit').disabled=!data?.authorized||data.serverAlive||data.lifecycleBusy||busy||chunkRecoveryBusy;document.querySelector('#restoreChunks').disabled=!data?.authorized||data.serverAlive||data.lifecycleBusy||busy||chunkRecoveryBusy||!chunkBackup.value||!chunkCoordinates.value.trim();document.querySelector('#rollbackChunks').disabled=!data?.authorized||data.serverAlive||data.lifecycleBusy||busy||chunkRecoveryBusy||!chunkTransaction.value}
