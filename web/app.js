...
const views=['overview','console','chat','players','items','world','commands','system','profiles','ai','users','maintenance'];
const initialView=views.includes(qs.get('view'))?qs.get('view'):'overview';

const titles={overview:'服务器总览',console:'实时日志',chat:'游戏聊天',players:'玩家管理',items:'物品资料库',world:'世界控制',commands:'命令中心',system:'本机资源监控',profiles:'服务器配置',ai:'游戏内 AI 助手',users:'Web 登录用户',maintenance:'维护与审计'};
...
function showView(name){
  ...
  if(name==='maintenance'){refreshAudit();refreshLifecycleStatus();refreshMaintenanceSchedule();refreshProgramUpdateStatus();refreshExecutionHistory()}
...
function renderServerStrip(){
  ...
  strip.innerHTML=lastStatus.servers.map(server=>`<button class="server-row ${server.id===selectedId?'selected':''} ${server.commandChannel==='readonly'?'readonly':''}" type="button" data-server-id="${escapeHtml(server.id)}"><span class="server-icon ${server.kind==='production'?'prod':'test'}"><i data-lucide="${server.kind==='production'?'server':'flask-conical'}"></i></span><span class="server-copy"><strong>${escapeHtml(server.name)}</strong><small>${escapeHtml(server.lanAddress||'未配置地址')} · ${server.commandChannel==='readonly'?'只读':'受控命令'}</small></span><span class="badge ${server.alive?'running':'stopped'}">${server.alive?'运行中':'已停止'}</span></button>`).join('');
...
const commandActionLabels={players:'查询在线玩家',connections:'查询连接列表',stats:'查询服务器统计',save:'保存当前世界',showoptions:'查询服务器选项',reloadoptions:'重新加载服务器选项','check-mod-updates':'检查 Mod 更新',help:'查询完整命令帮助','help-topic':'查询命令帮助',broadcast:'发送全服广播',additem:'发放物品',worldgen:'世界生成操作'};
...
function renderSystem(data){
  const host=data.host||{},netTotal=(data.network||[]).reduce((sum,item)=>sum+(Number(item.bytesPerSecond)||0),0);
