...
function describeMapResetProgress(progress){if(!progress)return'后台任务运行中，请勿关闭控制台进程。';const labels={'backup-scan':'正在统计完整存档','backup-write':'正在创建完整存档 ZIP','backup-verify':'正在逐文件校验备份 CRC','audit-report':'正在生成审计报告','reset-move':'正在隔离未保护区块','rollback-extract':'正在解压并验证完整备份','rollback-swap':'正在原子替换存档','completed':'操作已完成'},label=labels[progress.phase]||progress.message||'后台任务运行中',current=Number(progress.current)||0,total=Number(progress.total)||0;return total>0?`${label}：${current.toLocaleString('zh-CN')} / ${total.toLocaleString('zh-CN')}（${Math.min(100,Math.floor(current/total*100))}%）`:label}
