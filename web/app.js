...
function renderAntiCheatPlayers(){
  ...
  container.innerHTML=rows.length?rows.map(player=>{
    const signals=Number(player.protectedCalls||0)+Number(player.blockedCalls||0)+Number(player.observedHealthSyncs||0)+Number(player.nativeSignals||0)+Number(player.checksumSignals||0);
...
const mapResetForm=document.querySelector('#mapResetConfigForm'),mapResetAreaTable=document.querySelector('#mapResetAreaTable'),mapResetConfirmation=document.querySelector('#mapResetConfirmation'),mapResetRollbackPoint=document.querySelector('#mapResetRollbackPoint'),mapResetRollbackConfirmation=document.querySelector('#mapResetRollbackConfirmation');
...
function describeMapResetProgress(progress){if(!progress)return'后台任务运行中，请勿关闭控制台进程。';const labels={'backup-scan':'正在统计完整存档','backup-write':'正在创建完整存档 ZIP','backup-verify':'正在逐文件校验备份 CRC','audit-report':'正在生成审计报告','reset-move':'正在隔离未保护区块','region-invalidate':'正在失效旧区域缓存','reset-manifest':'正在提交车辆禁刷与区域重建清单','rollback-extract':'正在解压并验证完整备份','rollback-swap':'正在原子替换存档','completed':'操作已完成'},label=labels[progress.phase]||progress.message||'后台任务运行中',current=Number(progress.current)||0,total=Number(progress.total)||0;return total>0?`${label}：${current.toLocaleString('zh-CN')} / ${total.toLocaleString('zh-CN')}（${Math.min(100,Math.floor(current/total*100))}%）`:label}
...
function updateMapResetApplyState(){
  ...
  button.disabled=mapResetBusy||!data.authorized||data.serverAlive||data.lifecycleBusy||operationBusy||!data.resetGuard?.ready||!data.auditMatchesConfig||!confirmationOk;
  button.title=!data.authorized?'仅 Web admin 可执行':data.serverAlive?'必须先保存并停止游戏服务器':data.lifecycleBusy?'服务器生命周期操作仍在执行':operationBusy?'已有地图刷新任务正在执行':!data.resetGuard?.ready?(data.resetGuard?.message||'选择性地图重置运行时防护未就绪'):!data.auditMatchesConfig?'当前配置需要先完成一次只读审计':!confirmationOk?`请输入 ${data.serverName}`:'已满足执行条件';
  const rollbackConfirmationOk=mapResetRollbackConfirmation.value===data.serverName,backupSelected=Boolean(mapResetRollbackPoint.value);
  rollbackButton.disabled=mapResetBusy||!data.authorized||data.serverAlive||data.lifecycleBusy||operationBusy||!backupSelected||!rollbackConfirmationOk;
  rollbackButton.title=!data.authorized?'仅 Web admin 可执行':data.serverAlive?'必须先保存并停止游戏服务器':data.lifecycleBusy?'服务器生命周期操作仍在执行':operationBusy?'已有地图刷新任务正在执行':!backupSelected?'没有可用的完整备份':!rollbackConfirmationOk?`请输入 ${data.serverName}`:'已满足回滚条件';
}
function renderMapReset(data,hydrateConfig=false){
  ...
  if(hydrateConfig||mapResetServerId!==data.serverId){mapResetServerId=data.serverId;mapResetForm.elements.safehouseMarginChunks.value=data.config.safehouseMarginChunks;mapResetForm.elements.playerMarginChunks.value=data.config.playerMarginChunks;mapResetForm.elements.animalMarginChunks.value=data.config.animalMarginChunks;renderMapResetAreas(data.config.manualAreas||[],!data.authorized||operationBusy);mapResetConfirmation.value='';mapResetConfirmation.placeholder=data.serverName;mapResetRollbackConfirmation.value='';mapResetRollbackConfirmation.placeholder=data.serverName;renderMapResetRollbackPoints(data.rollbackPoints||[],false)}else renderMapResetRollbackPoints(data.rollbackPoints||[],true);
  ...
  mapResetForm.querySelectorAll('input').forEach(input=>input.disabled=!data.authorized||operationBusy);mapResetAreaTable.querySelectorAll('button').forEach(button=>button.disabled=!data.authorized||operationBusy);document.querySelectorAll('.map-reset-admin').forEach(control=>control.disabled=!data.authorized||operationBusy||mapResetBusy);mapResetConfirmation.disabled=!data.authorized||operationBusy;mapResetRollbackConfirmation.disabled=!data.authorized||operationBusy;mapResetRollbackPoint.disabled=!data.authorized||operationBusy||!(data.rollbackPoints||[]).length;
...
mapResetConfirmation.oninput=updateMapResetApplyState;mapResetRollbackConfirmation.oninput=updateMapResetApplyState;mapResetRollbackPoint.onchange=updateMapResetApplyState;
document.querySelector('#applyMapReset').onclick=async()=>{const data=mapResetSnapshot;if(!data||mapResetBusy||!data.resetGuard?.ready||mapResetConfirmation.value!==data.serverName)return;if(!confirm(`最终确认：对 ${data.serverName} 创建完整存档备份并执行选择性区块刷新？\n\n完整备份通过逐文件 CRC 校验后，工具会隔离地图区块和旧 IsoRegion 缓存，并提交原版车辆禁刷清单；服务器不会自动启动。`))return;mapResetBusy=true;try{const result=await api('/api/map-reset/apply',{method:'POST',body:JSON.stringify({serverId:selectedId,confirmation:mapResetConfirmation.value}),timeoutMs:30000});renderMapReset(result,false);toast(result.message)}catch(error){toast(error.message,true)}finally{mapResetBusy=false;if(mapResetSnapshot)renderMapReset(mapResetSnapshot,false)}};
document.querySelector('#rollbackMapReset').onclick=async()=>{const data=mapResetSnapshot,backupId=mapResetRollbackPoint.value;if(!data||mapResetBusy||!backupId||mapResetRollbackConfirmation.value!==data.serverName)return;const point=(data.rollbackPoints||[]).find(item=>item.id===backupId),label=point?`${formatDate(point.createdAt)}，${formatMapResetBytes(point.sourceBytes)}`:backupId;if(!confirm(`最终确认：将 ${data.serverName} 完整回滚到 ${label}？\n\n人物、车辆、交易数据和整个世界都会回到该时间点；刷新后的进度不会合并，但当前世界会先整体保留。`))return;mapResetBusy=true;try{const result=await api('/api/map-reset/rollback',{method:'POST',body:JSON.stringify({serverId:selectedId,backupId,confirmation:mapResetRollbackConfirmation.value}),timeoutMs:30000});renderMapReset(result,false);toast(result.message)}catch(error){toast(error.message,true)}finally{mapResetBusy=false;if(mapResetSnapshot)renderMapReset(mapResetSnapshot,false)}};
